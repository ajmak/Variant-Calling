...
platTotal <- read.delim(paste(platDir, 'totalBreakdown.txt', sep=""), sep="\t", header=TRUE, stringsAsFactors=FALSE)
varTotal <- read.delim(paste(varDir, 'totalBreakdown.txt', sep=""), sep="\t", header=TRUE, stringsAsFactors=FALSE)
#mutTotal <- read.delim(paste(mutDir, 'totalBreakdown.txt', sep=""), sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
platCounts <- platCounts %>% mutate(Tool="Platypus")
varCounts <- varCounts %>% mutate(Tool="VarDict")

## Bind data frames together (append) ##
sumBreakdown <- bind_rows(platCounts, varCounts)
#sumBreakdown <- sumBreakdown %>% group_by(Sum)  #gather(Variant.Type, Sum, Tool) %>% arrange(desc(Sum))

#positions <- sumBreakdown$Variant.Type
maxSum=max(sumBreakdown$Sum)
print(sumBreakdown)
########## Plot sum with IGR, 5'Flank, and Introns ##########
withIGRHisto <- ggplot(sumBreakdown, aes(x=Variant.Type, y=Sum, fill=Tool)) + geom_bar(stat="identity", position="dodge") + coord_flip() + geom_text(aes(label=Sum), hjust=-0.25, size=3, position = position_dodge(width = 1)) + theme_classic() + scale_x_discrete(limits=positions) + scale_y_continuous("Count", expand = c(0, 0), limits=c(0,maxSum+6000))
ggsave(withIGRHisto, filename="HistoWithIGR.jpg")
...
positions <- positions %>% filter(Variant.Type != "IGR" & Variant.Type != "Intron"  & Variant.Type !="X5.Flank")
maxSum=max(sumBreakdown$Sum)

noIGRHisto <- ggplot(sumBreakdown, aes(x=Variant.Type, y=Sum, fill=Tool)) + geom_bar(stat="identity", position="dodge") + coord_flip() + geom_text(aes(label=Sum), hjust=-0.25, size=3, position = position_dodge(width = 1)) + theme_classic() + scale_x_discrete(limits=positions) + scale_y_continuous("Count", expand = c(0, 0), limits=c(0,maxSum+6000))

ggsave(noIGRHisto, filename="HistoNoIGR.jpg")
...
jpeg(filename="sensitivityFPKM.jpg", width=10, height=6, units="in", res=300)
hist_top <- hist_top + labs(y="# Known Variants", x="") 
sensPlot <- ggplot(sensFPKM, aes(x=bin, y=Sensitivity, color=Tool)) + geom_point() + theme_classic() + scale_x_continuous(limits=c(-13,13), breaks = seq(-13, 13, by = 1)) + scale_y_continuous(limits=c(0,1), breaks = seq(0, 1, by = 0.1))
...
binMax <- 500 #max(covNotFound$V7)
print(binMax)
covFound$bin <- cut(covFound$V7, breaks=seq(0, binMax+20, by=20), labels=seq(0, binMax, by=20))
covNotFound$bin <- cut(covNotFound$V7, breaks=seq(0, binMax+20, by=20), labels=seq(0, binMax, by=20))
...
jpeg(filename="sensitivityLowCoverage.jpg", width=10, height=6, units="in", res=300)
maxCount=max(totCov$total)
histCov <- totCov %>% distinct(totCov, total, .keep_all=TRUE)

## plot histogram of known variants in each bin 
hist_top <- ggplot(histCov, aes(x=bin, y=total)) + geom_bar(stat="identity") + theme_classic() + scale_y_continuous(trans='log2', breaks=c(5,30,250,2050)) #+ scale_x_discrete(limits=c(0, maxCount+50), breaks = seq(0, maxCount+10)) + scale_y_continuous(limits=c(0, maxCount=50), trans='log2')
hist_top <- hist_top + labs(y="# Known Variants", x="")
## plot sensitivity vs read coverage
sensPlot <- ggplot(totCov, aes(x=bin, y=sensitivity, color=Tool)) + geom_point() + theme_classic() + scale_x_discrete(breaks = seq(0, binMax, by = 20)) + scale_y_continuous(limits=seq(0,1.0), breaks = seq(0, 1, by = 0.1))
